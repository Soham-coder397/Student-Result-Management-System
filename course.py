...
class Course:
    def __init__(self,root):
        ...
        #-----Title-----
        title=Label(self.root,text="Manage Course Details",compound=LEFT,font=("goudy old style",20,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1180,height=35)
        #-----Variables-----
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "":
                ...
                cur.execute("select * from Course where name LIKE ?", 
                            ('%' + self.var_search.get() + '%',))
                rows = cur.fetchall()
    ...
    def delete(self):
        ...
        try:
            if self.var_cid.get() == "":
                messagebox.showerror("Error", "Please select course first", parent=self.root)
            else:
