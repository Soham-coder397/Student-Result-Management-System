...
class Register:
    def __init__(self,root):
        ...
        frame1.place(x=460,y=100,width=700,height=500)
        ...
        #--------------Terms & Condition---------------------
        self.var_chk=IntVar()
        chk=Checkbutton(frame1,text="I Agree The Terms & Condition",variable=self.var_chk,onvalue=1,offvalue=0,font=("times new roman",12),bg="White",activebackground="white",cursor="hand2").place(x=50,y=390)
        self.btn_img=ImageTk.PhotoImage(file="images/register.png")
        btn_register=Button(frame1,image=self.btn_img,cursor="hand2",bd=0,command=self.register_data).place(x=50,y=430)
        btn_login=Button(self.root,text="Sign In",font=("times new roman",20),cursor="hand2",bd=0,bg="white",command=self.signin_window).place(x=200,y=460,width=180)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_lname.get()=="" or \
        self.txt_contact.get()=="" or self.txt_email.get()=="" or \
        ...
        self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "User already exists, try different email", parent=self.root)
                else:
                    cur.execute(
                        "INSERT INTO Users (f_name,l_name,contact,email,question,answer,password) VALUES (?,?,?,?,?,?,?)",
                        (
                            self.txt_fname.get(),
                            self.txt_lname.get(),
